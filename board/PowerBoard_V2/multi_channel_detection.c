...
OSStatus multi_channel_adc_dma_init( void )
{
  ...
  require_noerr_quiet( err, exit );
  err = MicoAdcStreamAddChannel( MICO_ADC_ROUTER, SAMPLE_CYCLE );
  require_noerr_quiet( err, exit );
...
uint16_t  read_channel_values_mV( mico_adc_t adc )
{
  ...
  else if ( adc > MICO_ADC_SWITCH && adc <=  MICO_ADC_VDET_BAT )
  {    
    for( uint8_t i = 0; i < FILTER_TIMES; i++ )
    {
...
{  
  ...
  printf("%d\t",(uint16_t)(read_channel_values_mV( MICO_ADC_ROUTER )* FIXIT_FACTOR));
