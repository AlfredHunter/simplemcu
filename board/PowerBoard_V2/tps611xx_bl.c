...
void startTps611xx( void )
{
  ...
  pin_config.gpio_mode = GPIO_MODE_OUTPUT_PP;
  pin_config.gpio_pull = GPIO_PULLUP;
  platform_gpio_init( bl_TPS61165.dev_driver->en_gpio, &pin_config );
  for(;;)
  {
    platform_gpio_output_low( bl_TPS61165.dev_driver->en_gpio );
    udelay(206);
    platform_gpio_output_high( bl_TPS61165.dev_driver->en_gpio );
    udelay(400);
  }
#endif
...
//  delay_us( 100 );
  for(;;)
  {
    ...
    delay_us( 5 );
    MicoGpioOutputHigh( (mico_gpio_t)MICO_GPIO_IRLED_PWM );
    delay_us( 15 );
  }
