...
int uart_ser_write( uart_serial_t *serial, const void *buffer, int length)
{
    int result = 0;
#ifndef COMM_DMA_USE_INT    
    while (HAL_UART_GetState(serial->uartHandle) != HAL_UART_STATE_READY)
    {
      serial->uartHandle->State = HAL_UART_STATE_READY;
    }
    if (HAL_UART_Transmit(serial->uartHandle, (uint8_t *)buffer, length,1) != HAL_OK)
    {
        result = -1;
    }
#else
...
int uart_ser_read( uart_serial_t *serial, void *buffer, int bufsize)
{
    ...
    if ( HAL_UART_Receive(serial->uartHandle, (uint8_t *)buffer, bufsize,1) != HAL_OK )
    {
        result = -1;
    }
#else
...
//    __HAL_UART_ENABLE( platform_uart_drivers[uart].uart_handle );
...
//    __HAL_UART_DISABLE( platform_uart_drivers[uart].uart_handle );
    __HAL_DMA_DISABLE( dma_handle );
